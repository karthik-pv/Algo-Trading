...
def load_json_file(filepath: str) -> Any:
    ...
    try:
        ...
        logger.error(f"Error: Could not decode JSON from the file '{filepath}'. Please check its format.")
        sys.exit(1)
...
def main():
    ...
    if not patterns_str:
        logger.error("Patterns cannot be empty.")
        sys.exit(1)
    ...
    if not isinstance(data, list):
        logger.error("Error: The JSON file must contain a list of objects at the top level.")
        sys.exit(1)
    ...
    # 4. Print the output
    if filtered_result:
        logger.info(f"\n--- Found {len(filtered_result)} matching items ---")
    else:
